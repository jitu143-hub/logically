...
module "gke" {
  ...
  name                       = "geotrade-app-org-cluster-${var.env_name}"
  region                     = var.gcp_region
  zones                      = ["us-east4-a", "us-east4-b", "us-east4-c"]
  network                    = data.terraform_remote_state.vpc.outputs.network_name
  subnetwork                 = "geotrade-app-org-subnet-dev"
  ip_range_pods              = "geotrade-app-org-ip-range-pods-dev"
  ip_range_services          = "geotrade-app-org-ip-range-services-dev"
  http_load_balancing        = false
  horizontal_pod_autoscaling = true
  network_policy             = false
  enable_private_endpoint    = false
  enable_private_nodes       = true
  master_ipv4_cidr_block     = var.app_org_master_ipv4_cidr_block
