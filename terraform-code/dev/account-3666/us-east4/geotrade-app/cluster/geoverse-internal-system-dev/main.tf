...
module "gke" {
  ...
  name                       = "geoverse-internal-system-dev"
  region                     = var.gcp_region
  zones                      = ["us-east4-a", "us-east4-b", "us-east4-c"]
  network                    = data.terraform_remote_state.vpc.outputs.network_name
  subnetwork                 = "geotrade-app-geoverse-internal-subnet-dev"
  ip_range_pods              = "geotrade-app-geoverse-internal-ip-range-pods-dev"
  ip_range_services          = "geotrade-app-geoverse-internal-ip-range-services-dev"
  http_load_balancing        = false
  horizontal_pod_autoscaling = true
  network_policy             = false
  enable_private_endpoint    = false
  enable_private_nodes       = true
  master_ipv4_cidr_block     = var.dev_geoverse_internal_master_ipv4_cidr_block
