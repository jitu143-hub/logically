...
resource "google_compute_network_peering" "peering-dev-geotrade-app-to-db-vpc" {
  name         = "peering-dev-geotrade-app-to-db-vpc"
  network      = data.terraform_remote_state.vpc-dev-app.outputs.network_id
  peer_network = data.terraform_remote_state.vpc-db-app.outputs.network_id
}
