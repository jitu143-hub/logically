...
module "firewall_rules" {
  source       = "/Users/pavanwadhe/Downloads/Mouritech Work/iac-geotrade-application/terraform-modules/firewall-rules"
  project_id   = var.project_id
  network_name = data.terraform_remote_state.vpc.outputs.network_name

  rules = [{
    name                    = "geotrade-app-central-ingress"
    description             = "opening port 8443 from master to nodes for ingress to work"
    direction               = "INGRESS"
    priority                = null
    ranges                  = [var.app_central_master_ipv4_cidr_block]
    source_tags             = null
