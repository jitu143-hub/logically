...
locals {
  ...
  actual_availability_type      = local.is_postgres && var.enable_failover_replica ? "ZONAL" : "REGIONAL"
...
resource "google_sql_database" "routers_db" {
...
resource "google_sql_user" "routers_db" {
  depends_on = [google_sql_database.routers_db]

  project  = var.project
  name     = "router_user"
  instance = google_sql_database_instance.master.name
  host     = local.is_postgres ? null : var.master_user_host
  password = "EXtgBA2eaRxe5PSB"
}
